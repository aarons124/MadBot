...
module.exports = {
  ...
  description: "Create a youtube-together sesion",
  ...
  run: async (client, message, args) => {
    try {
      ...
      client.discordTogether.createTogetherCode(channel, "youtube").then(async (invite) => {
        return message.reply({
          embeds: [
            new MessageEmbed()
              .setColor("#57F287")
              .setDescription(`${client.emotes.success} **YouTube-Together session created.** Click the button to join the activity`)
          ],
          components: [
      ...
      console.log(`[YOUTUBE_TOGETHER_COMMAND]: ${e}`);
    }
  }
}